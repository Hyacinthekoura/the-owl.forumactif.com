(function (root, factory) {
        ...
        ths.forEach((th) => {
            ...
            switch (col_type) {
                case "select":
                    ...
                    var thOptions = th.getAttribute("options").split(",") || [
                        "n/a",
                    ];
    ...
    function addLengthCounter({ form, table, submit_button }) {
        ...
        const base_length =
            uuidv4().length + 43 + "<td></td>".length * th_length;
    ...
    function postToHTML(resData) {
        return new Fetcher(
            ...
            .then((res) => {
                console.log(
                    htmlParser(res).querySelector("#main-content .errorbox")
                );
                location.reload();
            });
    }
    ...
    Plugin.prototype = {
        ...
        getCollection: function (key, comp, value, ...cols) {
            ...
            if (arguments.length >= 3) {
                return this.data.then((res) =>
                    queryItUp[comp](res.rows, key, value)
                );
            }
        },
    };
